...
const pendingDocuments = [
  {
    ...
    description:
      "Download template fill in your information and upload it here by May 5, 2025",
    type: "urgent",
    status: "pending",
    templateUrl: "/dsa.pdf",
  },
  {
    ...
    type: "urgent",
    status: "completed",
    templateUrl: "/src/assets/employee-details.pdf",
  },
  {
    id: 3,
    title: "Tax Information",
    description:
      "Submit your tax information using this template by May 15, 2025",
    type: "normal",
    status: "upcoming",
    templateUrl: "/src/assets/employee-details.pdf",
  },
  {
    ...
    description:
      "Complete your health insurance enrollment form by May 20, 2025",
    type: "urgent",
    status: "pending",
    templateUrl: "/src/assets/employee-details.pdf",
  },
  {
    ...
    description:
      "Set up your direct deposit information for payroll by May 12, 2025",
    type: "urgent",
    status: "completed",
    templateUrl: "/src/assets/employee-details.pdf",
  },
...
const DocumentCard = ({ document: doc }) => {
  ...
  const handleDownload = (e) => {
    ...
    e.stopPropagation();
    try {
      // Since the PDF is in the src/assets folder, we need to handle the path correctly
      const templatePath = doc.templateUrl;
  ...
  return (
    <div
      className={`w-[220px] h-[217px] border-2 ${statusStyles.border} rounded-[20px] p-4 flex flex-col`}
    >
      <div className="mb-4">
        <h3 className="font-semibold mb-1">{doc.title}</h3>
        <span
          className={`inline-block px-2 py-0.5 rounded-full text-xs ${statusStyles.bg} ${statusStyles.text} mb-2`}
        >
      ...
      <div className="mt-auto space-y-2">
        <button
          className={`w-[186px] h-8 text-white rounded-full flex items-center justify-center ${
            doc.status === "completed"
              ? "bg-green-500 hover:bg-green-600"
              : statusStyles.button
          }`}
        >
          {doc.status === "completed" ? "View Document" : "Upload"}
        </button>

        <button
          onClick={handleDownload}
          className="w-[144px] h-6 flex items-center justify-center text-blue-500 hover:bg-blue-50 rounded-full ml-[21px] text-xs"
        >
...
const Documents = () => {
  ...
  const renderContent = () => {
    switch (selectedOption) {
      case "manage":
        const pendingDocs = pendingDocuments.filter(
          (doc) => doc.status === "pending"
        );
        return (
          <div className="w-full h-full">
            <h2 className="text-xl font-semibold mb-6 sticky top-0 bg-white z-10">
              Pending Documents
            </h2>
      ...
      case "myDocuments":
        const completedDocs = pendingDocuments.filter(
  ...
  return (
    <div className="relative m-8 h-[945px] border border-gray-200 rounded-[25px] bg-white">
      ...
      <div className="absolute left-0 top-[60px] w-[250px] h-[calc(100%-60px)] border-r border-gray-200 p-4">
        ...
        <button
          className={`w-full py-2 px-4 mt-2 rounded-md text-left text-gray-800 text-sm ${
            selectedOption === "myDocuments"
              ? "bg-gray-100 hover:bg-gray-200 font-bold"
              : "hover:bg-gray-100"
          }`}
          onClick={() => setSelectedOption("myDocuments")}
        >
          My Documents
        </button>
        <button
          className={`w-full py-2 px-4 mt-2 text-left text-gray-800 text-sm ${
            selectedOption === "autofill"
              ? "bg-gray-100 hover:bg-gray-200 font-bold"
              : "hover:bg-gray-100"
          }`}
